...
from clip.a2_importance import calc_importance
from clip.a3_analyze import get_contrast_mask
...
def main(args):
    ...
    num_shards = data['train'].dataloader.num_shards
    def generate_masks():
        ...
        if is_master(args):        
            if args.unlearn_part:
                torch.save(forget_importances, os.path.join(mask_save_root, f"forget_importances_shards_{num_shards}_{args.unlearn_part}.pt"))
                torch.save(retain_importances, os.path.join(mask_save_root, f"retain_importances_shards_{num_shards}_{args.unlearn_part}.pt"))
            else:
                torch.save(forget_importances, os.path.join(mask_save_root, f"forget_importances_shards_{num_shards}.pt"))
                torch.save(retain_importances, os.path.join(mask_save_root, f"retain_importances_shards_{num_shards}.pt"))
        return forget_importances, retain_importances
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if args.unlearn_method == 'salun':
        ...
        elif args.unlearn_method in ['ft', 'ga', 'gaft']:
            ...
            unlearn_method(model, data, loss, epoch, optimizer, scaler, scheduler, dist_model, args, mask_layer=mask_layer, tokenizer=tokenizer, preprocess=preprocess_val)
        elif args.unlearn_method == 'calc_grad':
            unlearn_method(model, data, loss, epoch, optimizer, scaler, scheduler, dist_model, args)
            break
        elif args.unlearn_method == 'static':
            if args.unlearn_part:
                forget_importances = torch.load(mask_save_root/f'forget_importances_shards_{num_shards}_{args.unlearn_part}.pt', map_location='cpu')
                retain_importances = torch.load(mask_save_root/f'retain_importances_shards_{num_shards}_{args.unlearn_part}.pt', map_location='cpu')
            else:
                try:
                    forget_importances = torch.load(mask_save_root/f'forget_importances_shards_{num_shards}.pt', map_location='cpu')
                    retain_importances = torch.load(mask_save_root/f'retain_importances_shards_{num_shards}.pt', map_location='cpu')
                except:
                    print("generating importance masks...")
                    forget_importances, retain_importances = generate_masks()
            mask = get_contrast_mask(forget_importances, retain_importances, 0.5, part=args.unlearn_part)
            unlearn_method(model, data, loss, epoch, optimizer, scaler, scheduler, dist_model, args, mask=mask, tokenizer=tokenizer)
        elif args.unlearn_method == 'dynamic':
            if epoch == 0:
                try:
                    forget_importances = torch.load(mask_save_root/f'forget_importances_shards_{num_shards}.pt', map_location='cpu')
                    retain_importances = torch.load(mask_save_root/f'retain_importances_shards_{num_shards}.pt', map_location='cpu')
                    ...
                    forget_importances, retain_importances = generate_masks()
            mask = get_contrast_mask(forget_importances, retain_importances, 0.5, part=args.unlearn_part)
            forget_importances, retain_importances = unlearn_method(model, data, loss, epoch, optimizer, scaler, scheduler, dist_model, args, mask=mask, tokenizer=tokenizer)
            if epoch != 0:
                torch.save(forget_importances, mask_save_root/f'forget_importances_shards_{num_shards}_e{epoch}.pt')
                torch.save(retain_importances, mask_save_root/f'retain_importances_shards_{num_shards}_e{epoch}.pt')
